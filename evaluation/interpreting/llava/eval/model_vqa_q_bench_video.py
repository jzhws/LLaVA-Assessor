...
def eval_model(args):
    ...
    model.half()
    # Data
    for file in ['/tos-bjml-researcheval/jiaziheng/Q-bench-video-reb/Q_Bench_Video_test.json']:
        ...
        for i, llddata in enumerate((tqdm(llvqa_data))):
            video_path = llddata["video_name"].replace("/fs-computility/mllm1/zhangzicheng/VQA++/Visual-Question-Answering-for-Video-Quality-Assessment/VQA_benchmark_test/videos","/tos-bjml-researcheval/jiaziheng/VQA++/Visual-Question-Answering-for-Video-Quality-Assessment/VQA_benchmark_test/videos")
            if video_path is not None:
                if '_cat_' in video_path:
                    slice_len = []
                    image_files,frame_idx,frame_num=load_video('/tos-bjml-researcheval/jiaziheng/VQA++/Visual-Question-Answering-for-Video-Quality-Assessment/VQA_benchmark_test/videos/'+ video_path.split('_cat_')[0],1)
                    ...
                    image_files,frame_idx,frame_num1 = load_video(
                        '/tos-bjml-researcheval/jiaziheng/VQA++/Visual-Question-Answering-for-Video-Quality-Assessment/VQA_benchmark_test/videos/' + video_path.split('_cat_')[1][:-4],1)
                    image_tensor = image_processor.preprocess(image_files, return_tensors='pt')['pixel_values']
                    # image_tensor = torch.load(
                    #     "/fs-computility/mllm1/zhangzicheng/VQA++/q_bench_video/tensors/" +
                    ...
                    image_files,frame_idx,frame_num = load_video(
                        "/tos-bjml-researcheval/jiaziheng/VQA++/Visual-Question-Answering-for-Video-Quality-Assessment/VQA_benchmark_test/videos/" + video_path,1)
            ...
            if 'open_ended' in llddata['question_type']:
                ...
                message = message + "Please answer the question in the following format: the uppercase letter of the correct answer option itself +'.'. Please do not add any other answers beyond this."
            # 这一步处理比较问题
            # if 'Compare' in llddata['context'] or 'Joint' in llddata['context'] or "AIGC" in llddata["quality_concern"]:
            if 'Compare' in llddata['context'] or 'Joint' in llddata['context']:
                ...
                #     slice_len[0] + slice_len[1]) + " distinct frames in total. The first " + str(
                #     slice_len[0]) + " frames and " + str(slice_len[0] + 1) + "-" + str(slice_len[0] + slice_len[
                #     1]) + f"frames are uniformly sampled in each second from the first and the second video sequence, arranged in the same temporal order as they appear in the videos.  Please analyze these images and answer the questions based on your observations."
        ...
        #     qs = line["conversations"][0]["value"]
            ...
            with open(f"/tos-bjml-researcheval/jiaziheng/VQA++/Visual-Question-Answering-for-Video-Quality-Assessment/VQA_benchmark_test/{args.model_path.split('/')[-1]}_" + f"{file.split('/')[-1].split('_')[-1][:-5]}_answer.json", "w") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="/fs-computility/ResearchEval/shared/zihengjia/SFT-llava/llava-ov-chat-qwen2-VQA++_foundation")
