...
class LazySupervisedDataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in sources[0] and  "frames per sec" not in sources[0]:
            image_file = self.list_data_dict[i]["image"].replace("/tos-bjml-researcheval","/data/tos")
            if type(image_file) is list:
                ...
                if len(image_file) > 1:
                    ...
                    image = [([[im[0].repeat(4,1,1,1)],[im[0]]], im[1], "image") for im in image]
            else:
                image = self.process_image(image_file)
                image = [([[image[0].repeat(4,1,1,1)],[image[0]]], image[1], "image")]
            ...
            if 'B054' in self.list_data_dict[i]["image"]:
                video_file = self.list_data_dict[0]["image"].replace("/tos-bjml-researcheval","/data/tos")
            else:
                video_file = self.list_data_dict[i]["image"].replace("/tos-bjml-researcheval","/data/tos")
