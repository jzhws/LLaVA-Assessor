...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None):
        ...
        new_image_features = []
        images1 = images
        for visual_idx, images in enumerate(images1):
            cur_modality = modalities[visual_idx] if visual_idx < len(modalities) else modalities[-1]
            cur_image_size = image_sizes[visual_idx] if image_sizes is not None and visual_idx < len(image_sizes) else None
            if type(images) is list or images.ndim == 5:
                ...
                if mm_patch_merge_type == "flat":
                    ...
                    for image_idx, image_feature in enumerate(image_features):
                        ...
                        if cur_modality == "video":  # video operations
                            ...
                            if image_aspect_ratio == "anyres" or "anyres_max" in image_aspect_ratio:
                                ...
                                try:
                                    num_patch_width, num_patch_height = get_anyres_image_grid_shape(cur_image_size, self.config.image_grid_pinpoints, vision_tower_image_size)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                if cur_image_idx >= len(image_features):
                    raise ValueError(f"Not enough image features for text-only sample: cur_image_idx={cur_image_idx}, len(image_features)={len(image_features)}")
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    if cur_image_idx >= len(image_features):
                        raise ValueError(f"Not enough image features: cur_image_idx={cur_image_idx}, len(image_features)={len(image_features)}, num_images={num_images}")
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
        ...
        if cur_image_idx != len(image_features):
            raise ValueError(f"Unused image features: consumed={cur_image_idx}, len(image_features)={len(image_features)}")
