...
def eval_model(args):
    ...
    for image_path, json_ in zip(image_paths, jsons):
        ...
        #         iqadata = json.load(f)
            prs, gts = [], []
            for i, llddata in enumerate(tqdm(iqadata[1:], desc="Evaluating [{}]".format(json_.split("/")[-1]))):
                ...
                for name in os.listdir(image_path):
                    if name.split('_')[0] in filename:
                        image,frame_idx = load_video(image_path + name,24)
                        ...
                        image_tensors = [[image_tensors.cuda()], [image_tensor[frame_idx].half().cuda()]]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-path", type=str, default="./llava-ov-chat-qwen2_slowfast_instruction_direct_1")
