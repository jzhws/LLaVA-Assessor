...
def load_video(video_file, video_fps):
    ...
    for ii in range(len(vr)//round(vr.get_avg_fps())):
        ...
        avg_fps = round(vr.get_avg_fps() / video_fps+0.01)
    ...
    return [Image.fromarray(frames[i]) for i in list(range(len(frame_idx)))]
            ...
            for i, llddata in enumerate(tqdm(iqadata, desc="Evaluating [{}]".format(json_.split("/")[-1]))):
                ...
                for name in os.listdir(image_path):
                    if name.split('_')[0] in filename:
                        image = load_video(image_path + name,1)
                        inp = inp + "\n" + DEFAULT_IMAGE_TOKEN
                        # image = None
                        ...
                        # print(tokenizer.batch_decode(torch.argmax(output_logits, dim=-1).cpu().numpy(), skip_special_tokens=True)[0])
                        # print(torch.argmax(output_logits, dim=-1).cpu().numpy()[0])
                        output_logits = model(input_ids,
